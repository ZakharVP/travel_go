...
struct StoriesView: View {
    ...
    private let fullScreenStories: [Story] = [
        Story(
            imageName: "storyBigOne",
    ...
    var body: some View {
        ZStack {
            ...
            // Текущая сторис
            if selectedStoryIndex < fullScreenStories.count {
                let story = fullScreenStories[selectedStoryIndex]
                ...
                ZStack {
                    ...
                    VStack(spacing: 16) {
                        ...
                        .frame(width: 343, height: 208, alignment: .bottomLeading)
                        .padding(.bottom, 50)
                    }
                    ...
                    VStack {
                        HStack(spacing: 4) {
                            ForEach(0..<fullScreenStories.count, id: \.self) { index in
                                ProgressBar(progress: index == selectedStoryIndex ? progress : (index < selectedStoryIndex ? 1.0 : 0.0))
                                    .frame(height: 3)
                            }
                        }
                        .padding(.horizontal)
                        .padding(.top, safeAreaInsets.top + 8) // Учитываем safe area сверху
                        
                        HStack {
                            Text("\(selectedStoryIndex + 1)/\(fullScreenStories.count)")
                                .font(.system(size: 14))
                                .foregroundColor(.white)
    ...
    private func goToNextStory() {
        ...
        if selectedStoryIndex < fullScreenStories.count - 1 {
